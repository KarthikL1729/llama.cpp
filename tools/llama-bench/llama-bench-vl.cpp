...
static const cmd_params cmd_params_defaults = {
    ...
    /* prompt               */ "Describe the image in detail: <__media__>",
    /* images               */ {},
    /* mmproj               */ "",
    /* mmproj_use_gpu       */ true,
...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("\n");
        printf("model and backend parameters:\n");
    ...
    printf("\n");
        printf("multimodal input parameters:\n");
        printf("  --mmproj <filename>                       multimodal projector file (required)\n");
        printf("  --image <filename>                        image to benchmark (repeatable, required)\n");
        printf("  --prompt <text>                           prompt containing <__media__> markers (default: %s)\n",
            cmd_params_defaults.prompt.c_str());
        printf("  --prompt-file <filename>                  load prompt text from file (overrides --prompt)\n");
        printf("  --media-marker <token>                    override media marker (default: %s)\n",
            cmd_params_defaults.media_marker.c_str());
        printf("  --no-mmproj-offload                       disable GPU offload for mmproj\n");
        printf("  --image-min-tokens <n>                    minimum tokens for dynamic resolution models\n");
        printf("  --image-max-tokens <n>                    maximum tokens for dynamic resolution models\n");
        printf("\n");
        printf(
         "Multiple backend parameters can be given by separating values with ','\n"
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    params.prompt               = cmd_params_defaults.prompt;
    params.images               = cmd_params_defaults.images;
    params.mmproj               = cmd_params_defaults.mmproj;
    params.mmproj_use_gpu       = cmd_params_defaults.mmproj_use_gpu;
    params.media_marker         = cmd_params_defaults.media_marker;
    params.image_min_tokens     = cmd_params_defaults.image_min_tokens;
    params.image_max_tokens     = cmd_params_defaults.image_max_tokens;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "-h" || arg == "--help") {
                ...
                params.prompt = read_text_file(argv[i]);
            } else if (arg == "--image") {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
            ...
            } else if (arg == "--media-marker") {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
                }
                params.media_marker = argv[i];
            } else if (arg == "--no-mmproj-offload") {
    ...
    if (params.media_marker.empty()) {
        params.media_marker = cmd_params_defaults.media_marker;
    }
    if (params.images.empty()) {
        fprintf(stderr, "error: at least one --image must be provided\n");
        exit(1);
    }
...
static std::string ensure_prompt_markers(std::string prompt,
                                         const std::string & marker,
                                         size_t n_images) {
    size_t marker_count = count_media_markers(prompt, marker);
    if (marker_count == 0 && n_images > 0) {
        prompt += '\n';
        for (size_t i = 0; i < n_images; ++i) {
            prompt += marker;
        }
        marker_count = n_images;
    }
    if (marker_count != n_images) {
        throw std::runtime_error(string_format(
            "prompt contains %zu marker(s) but %zu image(s) provided", marker_count, n_images));
    }
    return prompt;
}

struct chunk_profile {
    size_t text_tokens  = 0;
    size_t media_tokens = 0;
    size_t media_chunks = 0;
};

static chunk_profile analyze_chunks(const mtmd_input_chunks * chunks) {
    ...
    for (size_t i = 0; i < n_chunks; ++i) {
        ...
        switch (mtmd_input_chunk_get_type(chunk)) {
            case MTMD_INPUT_CHUNK_TYPE_TEXT: {
                profile.text_tokens += mtmd_input_chunk_get_n_tokens(chunk);
            } break;
            case MTMD_INPUT_CHUNK_TYPE_IMAGE:
            case MTMD_INPUT_CHUNK_TYPE_AUDIO: {
                profile.media_tokens += mtmd_input_chunk_get_n_tokens(chunk);
                profile.media_chunks++;
            } break;
        }
    }
    return profile;
...
struct vl_run_metrics {
    uint64_t text_decode_ns   = 0;
    uint64_t media_encode_ns  = 0;
    uint64_t media_decode_ns  = 0;
};

static bool decode_text_chunk(llama_context * ctx,
                              llama_batch & batch,
    ...
    while (i < n_tokens) {
        batch.n_tokens = 0;
        for (; i < n_tokens && batch.n_tokens < batch_capacity; ++i) {
        ...
        metrics.text_decode_ns += get_time_ns() - start;
        if (ret != 0) {
            fprintf(stderr, "decode_text_chunk: llama_decode failed (%d)\n", ret);
            return false;
        }
    }
    return true;
...
static bool eval_chunks_vl(mtmd_context * mctx,
                           llama_context * ctx,
    ...
    const int32_t batch_capacity = n_batch;
    llama_pos   n_past     = 0;
    ...
    for (size_t i = 0; i < n_chunks; ++i) {
        ...
        if (chunk_type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
            ...
            if (!decode_text_chunk(ctx, text_batch, batch_capacity, tokens, n_tokens, n_past, seq_id, metrics)) {
                success = false;
                break;
            }
        } else if (chunk_type == MTMD_INPUT_CHUNK_TYPE_IMAGE || chunk_type == MTMD_INPUT_CHUNK_TYPE_AUDIO) {
            uint64_t start = get_time_ns();
            int32_t  ret   = mtmd_encode_chunk(mctx, chunk);
            metrics.media_encode_ns += get_time_ns() - start;
            if (ret != 0) {
                fprintf(stderr, "eval_chunks_vl: failed to encode media chunk\n");
                success = false;
                break;
            }
            ...
            start = get_time_ns();
            ret   = mtmd_helper_decode_image_chunk(mctx, ctx, chunk, embd, n_past, seq_id, n_batch, &new_n_past);
            metrics.media_decode_ns += get_time_ns() - start;
            if (ret != 0) {
                fprintf(stderr, "eval_chunks_vl: failed to decode media chunk\n");
                success = false;
                break;
            }
            n_past = new_n_past;
        } else {
            fprintf(stderr, "eval_chunks_vl: unsupported chunk type %d\n", (int) chunk_type);
            success = false;
            break;
        }
    }
...
static bool load_bitmaps(mtmd_context * ctx,
    ...
    media_names.clear();
    total_pixels = 0;
    out.entries.reserve(paths.size());
    media_names.reserve(paths.size());
    for (const auto & path : paths) {
        mtmd_bitmap * raw = mtmd_helper_bitmap_init_from_file(ctx, path.c_str());
        if (!raw) {
            return false;
        }
        mtmd::bitmap bmp(raw);
        const uint32_t nx = mtmd_bitmap_get_nx(raw);
        const uint32_t ny = mtmd_bitmap_get_ny(raw);
        total_pixels += (uint64_t) nx * ny;
        std::string name = std::filesystem::path(path).filename().string();
        bmp.set_id(name.c_str());
        media_names.push_back(name);
        out.entries.emplace_back(std::move(bmp));
    }
    return true;
}
...
struct test {
    ...
    size_t                   media_tokens = 0;
    size_t                   media_chunks = 0;
    uint64_t                 media_total_pixels = 0;
    std::string              media_files;
    std::string              test_time;
    std::vector<uint64_t>    samples_ns;
    std::vector<uint64_t>    samples_media_encode_ns;
    std::vector<uint64_t>    samples_media_decode_ns;
    std::vector<uint64_t>    samples_text_decode_ns;
    ...
    std::vector<double> get_ts() const {
        size_t              n_tokens = text_tokens + media_tokens;
    ...
    double avg_text_decode_ns() const { return ::avg(samples_text_decode_ns); }

    double stdev_text_decode_ns() const { return ::stdev(samples_text_decode_ns); }

    double avg_media_encode_ns() const { return ::avg(samples_media_encode_ns); }

    double stdev_media_encode_ns() const { return ::stdev(samples_media_encode_ns); }

    double avg_media_decode_ns() const { return ::avg(samples_media_decode_ns); }

    double stdev_media_decode_ns() const { return ::stdev(samples_media_decode_ns); }

    double avg_media_ts() const {
        if (media_tokens == 0 || samples_media_encode_ns.empty()) {
            return 0.0;
        }
        std::vector<double> totals(samples_media_encode_ns.size());
        for (size_t i = 0; i < totals.size(); ++i) {
            uint64_t enc = samples_media_encode_ns[i];
            uint64_t dec = i < samples_media_decode_ns.size() ? samples_media_decode_ns[i] : 0;
            totals[i] = enc + dec;
        }
        double avg_total = ::avg(totals);
        if (avg_total <= 0.0) {
            return 0.0;
        }
        return 1e9 * media_tokens / avg_total;
    }

    double avg_text_ts() const {
        if (text_tokens == 0 || samples_text_decode_ns.empty()) {
            return 0.0;
        }
        double avg_total = avg_text_decode_ns();
        if (avg_total <= 0.0) {
            return 0.0;
        }
        return 1e9 * text_tokens / avg_total;
    }
    ...
    static const std::vector<std::string> & get_fields() {
        static const std::vector<std::string> fields = {
            ...
            "no_host",        "media_files",    "media_pixels",  "media_chunks",   "text_tokens",
            "media_tokens",   "test_time",      "avg_ns",        "stddev_ns",      "avg_ts",
            "stddev_ts",      "avg_media_encode_ns", "stddev_media_encode_ns", "avg_media_decode_ns",
            "stddev_media_decode_ns",            "avg_text_decode_ns", "stddev_text_decode_ns",
            "media_ts",       "text_ts"
        };
        return fields;
    }
    ...
    static field_type get_field_type(const std::string & field) {
        if (field == "build_number" || field == "n_batch" || field == "n_ubatch" || field == "n_threads" ||
            ...
            field == "main_gpu" || field == "avg_ns" || field == "stddev_ns" || field == "no_op_offload" ||
            field == "n_cpu_moe" || field == "media_pixels" || field == "media_chunks" || field == "text_tokens" ||
            field == "media_tokens") {
            return INT;
        }
        ...
        if (field == "avg_ts" || field == "stddev_ts" || field == "avg_media_encode_ns" || field == "stddev_media_encode_ns" ||
            field == "avg_media_decode_ns" || field == "stddev_media_decode_ns" || field == "avg_text_decode_ns" ||
            field == "stddev_text_decode_ns" || field == "media_ts" || field == "text_ts") {
            return FLOAT;
        }
        return STRING;
    }
    ...
    std::vector<std::string> get_values() const {
        ...
        auto media_pixels_str = std::to_string(media_total_pixels);
        std::vector<std::string> values = { build_commit,
                                            ...
                                            media_files,
                                            media_pixels_str,
                                            std::to_string(media_chunks),
                                            std::to_string(text_tokens),
                                            std::to_string(media_tokens),
                                            test_time,
                                            std::to_string(avg_ns()),
                                            std::to_string(stdev_ns()),
                                            std::to_string(avg_ts()),
                                            std::to_string(stdev_ts()),
                                            std::to_string(avg_media_encode_ns()),
                                            std::to_string(stdev_media_encode_ns()),
                                            std::to_string(avg_media_decode_ns()),
                                            std::to_string(stdev_media_decode_ns()),
                                            std::to_string(avg_text_decode_ns()),
                                            std::to_string(stdev_text_decode_ns()),
                                            std::to_string(avg_media_ts()),
                                            std::to_string(avg_text_ts()) };
        return values;
    }
...
struct markdown_printer : public printer {
    ...
    static std::string get_field_display_name(const std::string & field) {
        ...
        if (field == "media_tokens") {
            return "m_tok";
        }
        if (field == "media_ts") {
            return "m_t/s";
        }
        if (field == "text_tokens") {
            return "txt_tok";
        }
        if (field == "text_ts") {
            return "txt_t/s";
    ...
    void print_header(const cmd_params & params) override {
        ...
        fields.emplace_back("media_tokens");
        fields.emplace_back("media_ts");
        fields.emplace_back("text_tokens");
        fields.emplace_back("text_ts");
    ...
    void print_test(const test & t) override {
        ...
        for (const auto & field : fields) {
            ...
            if (field == "model") {
                ...
                snprintf(buf, sizeof(buf), "txt%zu+img%zu", t.text_tokens, t.media_tokens);
                value = buf;
            } else if (field == "t/s") {
                snprintf(buf, sizeof(buf), "%.2f ± %.2f", t.avg_ts(), t.stdev_ts());
                value = buf;
            } else if (field == "media_tokens") {
                snprintf(buf, sizeof(buf), "%zu", t.media_tokens);
                value = buf;
            } else if (field == "media_ts") {
                snprintf(buf, sizeof(buf), "%.2f", t.avg_media_ts());
                value = buf;
            } else if (field == "text_tokens") {
                snprintf(buf, sizeof(buf), "%zu", t.text_tokens);
                value = buf;
            } else if (field == "text_ts") {
                snprintf(buf, sizeof(buf), "%.2f", t.avg_text_ts());
                value = buf;
...
int main(int argc, char ** argv) {
    ...
    try {
        prompt_text = ensure_prompt_markers(params.prompt, params.media_marker, params.images.size());
    } catch (const std::exception & e) {
    ...
    for (const auto & inst : params_instances) {
        params_idx++;
        if (params.progress) {
            fprintf(stderr, "llama-bench-vl: benchmark %d/%zu: starting\n", params_idx, params_count);
        }
        ...
        mparams.n_threads        = inst.n_threads;
        mparams.media_marker     = params.media_marker.c_str();
        mparams.flash_attn_type  = inst.flash_attn ? LLAMA_FLASH_ATTN_TYPE_ENABLED : LLAMA_FLASH_ATTN_TYPE_AUTO;
        mparams.image_min_tokens = params.image_min_tokens;
        mparams.image_max_tokens = params.image_max_tokens;

        mtmd::context_ptr mctx(mtmd_init_from_file(params.mmproj.c_str(), lmodel, mparams));
        if (!mctx) {
            fprintf(stderr, "%s: error: failed to initialize multimodal context '%s'\n",
        ...
        std::vector<std::string>  media_names;
        uint64_t                  media_pixels = 0;
        if (!load_bitmaps(mctx.get(), params.images, bitmaps, media_pixels, media_names)) {
            fprintf(stderr, "%s: error: failed to load media inputs\n", __func__);
            llama_model_free(lmodel);
            return 1;
        }
        ...
        int32_t tokenize_res = mtmd_tokenize(mctx.get(), chunks.ptr.get(), &text_input, bitmaps_ptr.data(), bitmaps_ptr.size());
        if (tokenize_res != 0) {
            fprintf(stderr, "%s: error: failed to tokenize prompt (code %d)\n", __func__, tokenize_res);
            llama_model_free(lmodel);
            return 1;
        }

        chunk_profile profile = analyze_chunks(chunks.ptr.get());

        llama_context_params cparams = inst.to_llama_cparams();
        uint32_t required_ctx = (uint32_t) (profile.text_tokens + profile.media_tokens + inst.n_batch + 512);
        cparams.n_ctx         = std::max<uint32_t>(cparams.n_ctx, required_ctx);
        ...
        t.text_tokens        = profile.text_tokens;
        t.media_tokens       = profile.media_tokens;
        t.media_chunks       = profile.media_chunks;
        t.media_total_pixels = media_pixels;
        t.media_files        = join(media_names, ";");
        ...
        if (!params.no_warmup) {
            if (params.progress) {
                fprintf(stderr, "llama-bench-vl: benchmark %d/%zu: warmup multimodal run\n", params_idx, params_count);
            }
            vl_run_metrics warmup_metrics{};
            if (!eval_chunks_vl(mctx.get(), ctx, chunks.ptr.get(), t.n_batch, t.n_threads, warmup_metrics)) {
                fprintf(stderr, "%s: error: failed to run warmup\n", __func__);
                exit(1);
            }
        }
        ...
        for (int i = 0; i < params.reps; i++) {
            ...
            if (params.progress) {
                fprintf(stderr, "llama-bench-vl: benchmark %d/%zu: run %d/%d\n", params_idx, params_count,
                        i + 1, params.reps);
            }

            vl_run_metrics metrics{};
            uint64_t       t_start = get_time_ns();
            bool           ok      = eval_chunks_vl(mctx.get(), ctx, chunks.ptr.get(), t.n_batch, t.n_threads, metrics);
            uint64_t       t_ns    = get_time_ns() - t_start;
            if (!ok) {
                fprintf(stderr, "%s: error: failed to execute multimodal run\n", __func__);
                exit(1);
            }
            t.samples_ns.push_back(t_ns);
            t.samples_media_encode_ns.push_back(metrics.media_encode_ns);
            t.samples_media_decode_ns.push_back(metrics.media_decode_ns);
            t.samples_text_decode_ns.push_back(metrics.text_decode_ns);
        }
